        ...
        Win10 = {
            ...
            _iframeOnClick: {
                ...
                Iframe: function () {
                    ...
                    _that.element = arguments[0];
                    _that.cb = arguments[1];
                    _that.hasTracked = false;
                },
                track: function (element, cb) {
                    var _that = Win10._iframeOnClick;
                    _that.iframes.push(new Win10Iframe(element, cb));
                    if (!_that.interval) {
                        _that.interval = setInterval(function () { _that.checkClick(); }, _that.resolution);
                    }
                },
                checkClick: function () {
                    if (document.activeElement) {
                        ...
                        for (var i = _that.iframes.length-1; i >=0;i--) {
                            ...
                            if ((eid = _that.iframes[i].element.id) && !document.getElementById(eid)) {
                                delete _that.iframes[i];
                                continue;
                            }
                            if (activeElement === _that.iframes[i].element) { // user is in this Iframe
                                if (_that.iframes[i].hasTracked === false) {
                                    _that.iframes[i].cb.apply(window, []);
                                    _that.iframes[i].hasTracked = true;
                                }
                            } else {
                                _that.iframes[i].hasTracked = false;
                            }
                        }
                    }
                }
            ...
            _renderBar: function () {
                //调整任务栏项目的宽度
                if (Win10._countTask <= 0) { return; } //防止除以0
                var btns = $("#win10_btn_group_middle>.btn");
                btns.css('width', ('calc(' + (1 / Win10._countTask * 100) + '% - 1px )'))
            },
            _handleReady: [],
            ...
            _settop: function (layero) {
                if (!isNaN(layero)) {
                    layero = Win10.getLayeroByIndex(layero);
                }
                //置顶窗口
                var max_zindex = 0;
            ...
            openUrl: function (url, title, areaAndOffset) {
                ...
                var area, offset;
                if (Win10.isSmallScreen() || areaAndOffset === 'max') {
            ...
            setAnimated: function (animated_classes, animated_liveness) {
                Win10._animated_classes = animated_classes;
