...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function (exports) {
    ...
    Class.prototype.pullData = function (curr, loadIndex) {
        ...
        if (options.url)
        { //Ajax请求
            options.getFilters = options.getFilters||function () {};
            var params = {},
                _url = options.url + "?",
                _filtes = options.getFilters();

            params[request.pageName] = curr;
            params[request.limitName] = options.limit;

            _url += _filtes ? "$filter=" + _filtes + "&" : "";
            _url += "$skip=" + ((curr - 1) * options.limit);
            _url += "&$top=" + options.limit;
            _url += "&$count=true";
